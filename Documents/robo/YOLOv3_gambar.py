...
images_path = glob.glob(r"8.jpeg") #ganti dengan direktori gambar anda
layer_names = net.getLayerNames()

# Fix the layer indexing
try:
...
for img_path in images_path:
    ...
    for i in range(len(counts)):
        if i < len(classes):  # Ensure index doesn't go out of range
            cv2.putText(img, str(classes[i]) + " = " + str(counts[i]), (5, 15 + tambah), font, 1, (0, 0, 255), 1)
            tambah = tambah + 15
        else:
            print(f"[WARNING] Skipping out-of-range class ID: {i}")
    ...
    for i in range(len(boxes)):
        if i in indexes:
            x, y, w, h = boxes[i]
            if class_ids[i] < len(classes):  # Ensure class_ids[i] is within the bounds of the class list
                ...
                print(f"[WARNING] Class ID {class_ids[i]} is out of range. Skipping this detection.")
